...
addon = xbmcaddon.Addon('plugin.video.FAMILIA JGP ADDONS ')
...
SOURCES = [{"title": "FAMILIA JGP ADDONS  ", "url": "http://familiajgpaddons.blogspot.com.es/p/version1.html", "fanart": "https://1.bp.blogspot.com/-sQL3Nn14B34/WPaxGeQkrGI/AAAAAAAAAAc/5-LXILRSdoo5nBn-h2WygqsuR_wEmycrgCLcB/s1600/429926470_213809.jpg", "genre": "Tv En directo", "date": "19.04.2017", "credits": "FAMILIA JGP ADDONS ", "thumbnail": "https://3.bp.blogspot.com/-Y8-5WoOvTy8/WPaxF7tIrqI/AAAAAAAAAAM/Zj7iM7uJgeoB2roWqbe0Wi-gVA6z_F0vwCLcB/s1600/429730858_225708.jpg"}]

# http://i.imgur.com/JqgYQf9.jpg
# http://i.imgur.com/5vfO2E9.png
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.FAMILIA JGP ADDONS -%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(FAMILIA JGP ADDONS ,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(FAMILIA JGP ADDONS ,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def getSources():
        if os.path.exists(favorites) == True:
            ...
            if FAV == "[]":
                ...
                addDir('[COLOR cyan][B]- Mis Canales Favoritos FAMILIA JGP ADDONS  [/COLOR][/B][/B]','url',4,os.path.join(home, 'resources', 'favorite.png'),FANART,'','','','')
                addDir('','',100,'',FANART,'','','','')
...
def urlsolver(url):
    ...
    if host:
        ...
        xbmc.executebuiltin("XBMC.Notification(FAMILIA JGP ADDONS ,Urlresolver donot support this domain. - ,5000)")
        resolver=url
    return resolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('FAMILIA JGP ADDONS ','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('FAMILIA JGP ADDONS ', 'Do you want to add this file as a source?')
        if ret:
...
#            print 'parentalblockedpin',parentalblockedpin
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from FAMILIA JGP ADDONS  Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
            ...
            if not name in FAV:
                contextMenu.append(('Add to FAMILIA JGP ADDONS  Favorites','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
#                print 'setting as folder in link'
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from FAMILIA JGP ADDONS  Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to FAMILIA JGP ADDONS  Favorites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
...
#            ln+='</item>'
        ...
        if url:
            ...
            xbmc.executebuiltin("XBMC.Notification(FAMILIA JGP ADDONS ,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(FAMILIA JGP ADDONS ,Please [COLOR yellow]install Youtube-dl[/COLOR] module ,10000,"")")
    ...
    if not (keyboard.isConfirmed() == False):
        ...
        if newStr==parentalblockedpin:
            ...
            xbmc.executebuiltin("XBMC.Notification(FAMILIA JGP ADDONS ,Parental Block Disabled,5000,"+icon+")")
        else:
            xbmc.executebuiltin("XBMC.Notification(FAMILIA JGP ADDONS ,Wrong Pin??,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
elif mode==56:
    addon_log("disable lock")
    addon.setSetting('parentalblocked', "true")
    xbmc.executebuiltin("XBMC.Notification(FAMILIA JGP ADDONS ,Parental block enabled,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
