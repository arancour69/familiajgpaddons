...
def dolist(rows=[],tipo2=False,zid=''):
    ...
    for row in rows:
        ...
        srv = '%s%s' %(srv,zid)
        url = '{0}?action=play&server={1}&user={2}&cole={3}&peli={4}&tit={5}'.format(_url, srv, usr, col, pel, base64.urlsafe_b64encode(tit.encode('utf-8')))
        is_folder = False
        listing.append((url, list_item, is_folder))
...
def play_video(server,user,cole,pel,tit):
    cinecrypt.play_video(server,user,cole,pel,tit,base)
...
def router(paramstring):
    ...
    if params:
        ...
        if params['action'] == 'tema2':
            ...
            play_video(params['server'],params['user'],params['cole'],params['peli'],params['tit'])
    else:
        list_categories()
